...
for item in data:
    ...
    colfrom = crane[1] - 1
    colto  = crane[2] - 1
    length = len(crates[colfrom])
    slicefrom = crates[colfrom][length-index:length]
    crates[colfrom] = crates[colfrom][0:length-index]
    slicefrom.reverse()
    loop_index = len(slicefrom)
    while loop_index != 0:
        popped = slicefrom.pop()
        crates[colto].append(popped)
        loop_index -= 1
